...
static struct CommandLineParserSpecification command_line_spec[] = {
    ...
    { 'd', "decode", "Decode mode (IMA-ADPCM wav -> PCM wav)",
        COMMAND_LINE_PARSER_FALSE, NULL, COMMAND_LINE_PARSER_FALSE },
    { 'r', "residual", "Residual output mode (PCM wav -> Residual PCM wav)",
        COMMAND_LINE_PARSER_FALSE, NULL, COMMAND_LINE_PARSER_FALSE },
...
static int do_residual_output(const char *wav_file, const char *residual_filename)
{
    ...
    if ((api_result = MOIDecoder_DecodeWhole(decoder, 
                    buffer, output_size, pcmdata, num_channels, num_samples)) != MOI_APIRESULT_OK) {
        fprintf(stderr, "Failed to decode. API result: %d \n", api_result);
        return 1;
    }
...
int main(int argc, char **argv)
{
    ...
    } else if (CommandLineParser_GetOptionAcquired(command_line_spec, "residual") == COMMAND_LINE_PARSER_TRUE) {
        /* 残差出力処理実行 */
        if (do_residual_output(input_file, output_file) != 0) {
            fprintf(stderr, "%s: failed to encode %s. \n", argv[0], input_file);
            return 1;
        }
    } else {
